...
public class WoWCharacter {
	...
	public WoWCharacter(String server, String name) throws IOException, BadCharacterException, NoAPIKeyException {
		...
		this.gameClass = ((HashMap<Integer, String>) Bot.getInstance().getCache().get("wowclasses")).get(jo.get("class").asInt());
		this.race = ((HashMap<Integer, String>) Bot.getInstance().getCache().get("wowraces")).get(jo.get("race").asInt());
	...
	static {
		...
		try {
			...
			Bot.getInstance().getCache().put("wowclasses", classes);
		} catch (IOException | NoAPIKeyException e) {
			Bot.getInstance().getLogger().warn("You do not have a valid Battle.net API key.\nAll Battle.net services are disabled");
		}
	}
}
