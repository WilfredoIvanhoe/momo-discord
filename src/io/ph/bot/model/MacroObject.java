...
public class MacroObject {
	 ...
	 * @param fallbackUsername IUser#getName()
	...
	public static MacroObject forName(String name, String guildId) throws NoMacroFoundException {
	 ...
	 */
	public static MacroObject forName(String name, String guildId, boolean hit) throws NoMacroFoundException {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			...
			rs = stmt.executeQuery();
			if(!rs.isBeforeFirst()) {
				throw new NoMacroFoundException();
			}
			rs.next();
			if(hit) {
